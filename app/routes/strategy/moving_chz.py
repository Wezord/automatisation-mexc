...
moving_chz_bp = Blueprint('moving_chz', __name__)

@moving_chz_bp.route('/moving_chz', methods=['GET'])
def moving_chz():
    if request.method == "GET" and request.headers.get("X-Custom-Message") == "get_alert":
        if current_app.config["current_alert"] == []:
            print("Vide sur moving_chz")
            return jsonify({"strategies": []}), 200
        else :
            open_position = mapi.get_all_open_position("moving_chz")
            print("Nombre d'ordre ouvert", len(open_position))
            print(current_app.config["open_position_count"]["moving_chz"])
            if len(open_position) > current_app.config["open_position_count"]["moving_chz"]:
                strategies_to_send = [
                    {'strategy_order_name': "moving_chz", 'type': 'sell', 'position': "long" if actif.split(".")[1] == "2" else "short" ,'alert_message': 'Force Exit', 'actif': actif, 'stop_loss': '0', 'time': '2025-01-11T17:54:00Z'} 
                    for actif in open_position]
                return jsonify({"strategies": strategies_to_send}), 200
            strategies_to_send = []
            for d in current_app.config['current_alert']:
                if d["strategy_order_name"] == "moving_chz":
                    ...
                    if  not (
                        ...
                        if d["type"] == "sell":
                            current_app.config['open_position_count']["moving_chz"] = current_app.config['open_position_count']["moving_chz"] - 1
                        elif d["type"] == "buy":
                            current_app.config['open_position_count']["moving_chz"] = current_app.config['open_position_count']["moving_chz"] + 1
                    else:
    ...
    return render_template('moving_chz.html', strategies = current_app.config['current_alert'])