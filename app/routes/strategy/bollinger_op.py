...
bollinger_op_bp = Blueprint('bollinger_op', __name__)

@bollinger_op_bp.route('/bollinger_op', methods=['GET'])
def bollinger_op():
    if request.method == "GET" and request.headers.get("X-Custom-Message") == "get_alert":
        if current_app.config["current_alert"] == []:
            print("Vide sur bollinger_op")
            return jsonify({"strategies": []}), 200
        else :
            open_position = mapi.get_all_open_position("bollinger_op")
            print("Nombre d'ordre ouvert", len(open_position))
            print(current_app.config["open_position_count"]["bollinger_op"])
            if len(open_position) > current_app.config["open_position_count"]["bollinger_op"]:
                strategies_to_send = [
                    {'strategy_order_name': "bollinger_op", 'type': 'sell', 'position': "long" if actif.split(".")[1] == "2" else "short" ,'alert_message': 'Force Exit', 'actif': actif, 'stop_loss': '0', 'time': '2025-01-11T17:54:00Z'} 
                    for actif in open_position]
                return jsonify({"strategies": strategies_to_send}), 200
            strategies_to_send = []
            for d in current_app.config['current_alert']:
                if d["strategy_order_name"] == "bollinger_op":
                    ...
                    if  not (
                        ...
                        if d["type"] == "sell":
                            current_app.config['open_position_count']["bollinger_op"] = current_app.config['open_position_count']["bollinger_op"] - 1
                        elif d["type"] == "buy":
                            current_app.config['open_position_count']["bollinger_op"] = current_app.config['open_position_count']["bollinger_op"] + 1
                    else:
    ...
    return render_template('bollinger_op.html', strategies = current_app.config['current_alert'])